...
async function parseComponentFile(filePath: string): Promise<ComponentInfo | null> {
  try {
    ...
    // Extract signals
    const signals: Record<string, SignalMetadata> = {}
 ...
 */
export async function getComponentSchema(name: string): Promise<ComponentInfo | null> {
 ...
 */
export function getComponentPattern(): string {
