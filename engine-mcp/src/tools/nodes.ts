...
let cachedNodes: NodeTypeDefinition[] | null = null
 ...
 */
async function parseNodesFile(): Promise<NodeTypeDefinition[]> {
  ...
  for (const block of nodeBlocks) {
    try {
      ...
      const category = block.match(/category:\s*['"]([^'"]+)['"]/)?.[1] as NodeCategory
 ...
 */
function parsePortsArray(block: string, arrayName: string): NodePortDefinition[] {
  ...
  while ((match = portRegex.exec(arrayContent)) !== null) {
    ...
    const type = portBlock.match(/type:\s*['"]([^'"]+)['"]/)?.[1] as NodePortDefinition['type']
 ...
 * List all node types
 */
export async function listNodeTypes(): Promise<NodeTypeDefinition[]> {
 ...
 */
export async function getNodesByCategory(category: NodeCategory): Promise<NodeTypeDefinition[]> {
 ...
 */
export async function searchNodes(query: string): Promise<NodeTypeDefinition[]> {
 ...
 * Get node by ID
 */
export async function getNodeType(id: string): Promise<NodeTypeDefinition | null> {
  const nodes = await parseNodesFile()
 ...
 */
export async function getNodeStats(): Promise<Record<string, number>> {
  const nodes = await parseNodesFile()
 ...
 */
export function getNodeExecutorPattern(): string {
 ...
 */
export function getNodeCategories(): Array<{ id: NodeCategory; name: string; color: string; description: string }> {
  return [
