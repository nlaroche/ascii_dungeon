...
import { useTheme, useTemplate, useEngineState } from '../stores/useEngineState';
...
function MenuDropdown({ items, onClose }: MenuDropdownProps) {
  ...
  return (
    <div
      className="absolute left-0 top-full z-50 min-w-52 rounded-md"
      style={{
        ...
        item.separator ? (
          ...
          >
            <button
              ...
              className="w-full px-3 py-2.5 text-left flex items-center justify-between transition-colors rounded"
              style={{
            ...
            >
              ...
              {item.shortcut && (
                <span className="ml-4 text-xs" style={{ color: theme.textDim }}>{item.shortcut}</span>
              )}
              {item.submenu && (
            ...
            {item.submenu && hoveredSubmenu === item.label && (
              <div
                className="absolute left-full top-0 min-w-44 rounded-md"
                style={{
                  backgroundColor: menuBg,
                  border: `1px solid ${menuBorder}`,
                  boxShadow: '0 8px 32px rgba(0, 0, 0, 0.6), 0 2px 8px rgba(0, 0, 0, 0.4)',
                ...
                {item.submenu.map((subitem, subidx) =>
                  subitem.separator ? (
                    ...
                    <button
                      ...
                      className="w-full px-3 py-2.5 text-left transition-colors rounded"
                      style={{
                        backgroundColor: 'transparent',
